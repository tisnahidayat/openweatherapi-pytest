...
def test_by_city(api_client):
    ...
    assert isinstance(json_response["main"]["temp"], float), "Response 'main.temp' should be a float"
    assert isinstance(json_response["wind"]["speed"], float), "Response 'wind.speed' should be a float"
    assert isinstance(json_response["wind"]["deg"], int), "Response 'wind.deg' should be an integer"
    assert isinstance(json_response["name"], str), "Response 'name' should be a string"

@pytest.mark.positive
def test_by_coordinate(api_client):
...
def test_invalid_zip_code(api_client):

    response = api_client.get("data/2.5/weather", params={"id": "9999999"}) # Example of invalid coordinates
